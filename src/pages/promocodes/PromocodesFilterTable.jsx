...
export const PromocodesFilterTable = () => {
  const {
    updateTasksList,
    filterOptions,
    addFilter,
  ...
  const searchOptions = [
    { label: "Общий поиск", value: "" },
    { label: "ID", value: "promocode_id" },
  ...
  const handleFilterOption = () => {
    // Удаляем все возможные ключи поиска
    removeFilter("promocode_id");
    removeFilter("name");
    removeFilter("search");

    // Если выбран конкретный параметр поиска
    if (searchKey && search.trim()) {
      addFilter(searchKey, search.trim());
    }
    // Если не выбран параметр, но есть текст поиска - используем общий search
    else if (search.trim()) {
