...
const TicketChartCard = ({
  ...
  const giveawaysData = [
    { id: 0, name: "Получено", value: "RECEIVED" },
    { id: 1, name: "Потрачено", value: "SPENT" },
  ];
  ...
  useEffect(() => {
    const getStatisticsData = async () => {
      try {
        const preset = giveawaysData[selectedGiveaway]?.value || "RECEIVED";
        const start = dayjs(dateRange[0]).format("YYYY-MM-DDTHH:mm");
        const end = dayjs(dateRange[1]).format("YYYY-MM-DDTHH:mm");

        const url = `${BASE_URL}/dashboards/graphs/tickets?start=${start}&end=${end}&preset=${preset}`;
  ...
  return (
    ...
    >
      ...
      <div className="flex lg:hidden flex-col gap-2 items-center border-b px-8 pb-2">
